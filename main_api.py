...
def get_tasks(status: ValidStatuses | None = None):
...
def update_task_description(task_id: int, task_update: TaskUpdate):
...
def update_task_status(task_id: int, status: TaskStatusUpdate):
